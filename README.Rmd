...
resolved device-pixel box; `vellumplot` declares which marks are interactive; and
...
series visibility) · continuous colorbar filter (drag a value range on the
colorbar to fade out-of-range marks) · optional overview navigator (`navigator = TRUE`: a
...
`group` (selection, hover, and pan/zoom). Each is opt-outable via an
`as_widget()` argument.
