...
`vellum` scene. Declare interactivity in `quill` with the reserved mark
arguments `data_id` (the join key), `tooltip`, and `hover_group` (links
elements for shared highlighting); discrete `color`/`shape` legends then
become interactive automatically. A plot that declares none renders as a
static — but still embeddable — SVG.
...
**Own bus (gloss ↔︎ gloss, no dependency).** Give the widgets a shared
`group`:
