...
element table, and bundles them with the `gloss` JavaScript runtime into
a self-contained
...
- x:
  ...
  A `quill` plot (a `PlotSpec` / `PlotComposition`) or a `vellum` scene
  — anything
...
- alt:
  ...
  scene's own title/description — which `quill` sets automatically from
  the plot title and
  [`quill::plot_alt()`](https://rdrr.io/pkg/quill/man/plot_alt.html) —
...
- hover_color, selected_color:
  ...
  per-mark `hover_color`/`selected_color` declared in `quill` overrides
  these for that mark.
...
An htmlwidget of class `"gloss"`.

## Details

Interactivity is driven by the keys/metadata a plot declares. In `quill`
these come from the reserved `data_id` / `tooltip` / `hover_group` mark
arguments; a plot that declares none renders as a static (but still
embeddable) SVG. A hovered element with a `data_id` but no `tooltip`
shows its key.

The scene metadata `gloss` reads — the
[`vellum::scene_model()`](https://rdrr.io/pkg/vellum/man/scene_model.html)
