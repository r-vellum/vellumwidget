...
the `viewBox` and the element geometry lines up with it without any
...
*Exact picking on top of it.* The R-tree indexes bounding boxes, which
are the right shape for a rectangular brush and the wrong one for
deciding what the cursor is pointing at: a rising line’s box is the
whole rectangle its endpoints span. So the R-tree shortlists candidates
— a box distance is never greater than the distance to the shape inside
it, so nothing in range is missed — and those candidates are then ranked
by distance to the mark’s real geometry, which the payload carries in
device pixels. A round mark is measured to its disc rather than the
square around it, and that costs nothing extra: the disc is the bounding
box’s centre and half-extent, so no per-point geometry is shipped for a
scatter. Only the kinds whose shape is not their box (segments, lines,
polygons, paths) add anything to the payload.
