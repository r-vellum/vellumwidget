...
The widget itself draws nothing. `vellum` hands it the rendered SVG plus
`scene_model()`, one row per drawn element carrying that element’s data
key and its resolved device-pixel box, and everything below is that
table plus event handlers. Two things follow. What you interact with is
the *same* scene as the static figure, not a second rendering of your
plot. And gestures that need geometry rather than styling — brushing a
rectangle, zooming to a region, stepping between marks with the arrow
keys — are table lookups rather than new drawing code.
